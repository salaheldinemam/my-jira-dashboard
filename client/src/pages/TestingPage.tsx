...
function IssueTable({
  ...
  const ticketTdClass =
    variant === "failed"
      ? "px-3 py-2 font-mono text-sky-800 dark:text-sky-200 w-40 min-w-[10rem]"
      : "px-3 py-2 font-mono text-sky-800 dark:text-sky-200";
  return (
        ...
        <tbody>
          {issues.map((i) => (
            <tr key={i.key} className="app-table-row">
              ...
              {showReporterCol && <td className="px-3 py-2">{i.reporter}</td>}
              {showAssigneeCol && <td className="px-3 py-2">{i.assignee}</td>}
              {showUpdated && <td className="px-3 py-2 text-app-text-muted text-xs">{i.updated}</td>}
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <div className="flex gap-1 mb-4 border-b border-slate-200 dark:border-slate-800">
        <button
          ...
          className={`px-4 py-2 text-sm font-medium rounded-t-md border-b-2 -mb-px transition-colors ${
            tab === "failed"
              ? "border-rose-500 text-slate-900 dark:text-white bg-white/80 dark:bg-slate-900/80"
              : "border-transparent text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-800 dark:text-slate-200"
          }`}
        >
          Failed testing
        </button>
        <button
          ...
          className={`px-4 py-2 text-sm font-medium rounded-t-md border-b-2 -mb-px transition-colors ${
            tab === "under"
              ? "border-sky-500 text-slate-900 dark:text-white bg-white/80 dark:bg-slate-900/80"
              : "border-transparent text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-800 dark:text-slate-200"
          }`}
        >
          Under testing
        ...
        <button
          ...
          className={`px-4 py-2 text-sm font-medium rounded-t-md border-b-2 -mb-px transition-colors ${
            tab === "ready"
              ? "border-emerald-500 text-slate-900 dark:text-white bg-white/80 dark:bg-slate-900/80"
              : "border-transparent text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-800 dark:text-slate-200"
          }`}
        >
          Ready for testing
