...
export function WorkloadPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-semibold text-white mb-2">Workload</h1>
      <p className="text-slate-400 text-sm mb-6">Tickets per assignee, grouped by Jira status.</p>
      ...
      {!loading && !error && (
        <div className="space-y-3">
          {Object.entries(byAssignee).map(([emp, buckets]) => (
            <div key={emp} className="rounded-xl border border-slate-800 bg-slate-900/40 overflow-hidden">
              <button
                type="button"
                className="w-full text-left px-4 py-3 flex justify-between items-center hover:bg-slate-800/60"
              ...
              {openEmp === emp && (
                <div className="border-t border-slate-800 px-4 py-3 space-y-4 bg-slate-950/50">
                    ...
                    return (
                      <div key={status}>
                        ...
                        <ul className="space-y-1 text-sm">
                          {list.map((issue) => (
                            <li key={issue.key} className="flex flex-wrap gap-2 text-slate-300">
                              <JiraIssueLink
                                issueKey={issue.key}
